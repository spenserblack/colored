...
- Implemented `ColorLevel`:
  - `ColorLevel` has the following values: `None` `Ansi16` `Ansi256` `TrueColor`.
  - Enhanced terminal color-support detection, with a particular focus on Windows Terminal.
- Changes to `Color`:
  - Implemented `Ansi256`.
